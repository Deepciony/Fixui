<script lang="ts">
  ...
  const translations = {
    th: {
      ...
      draft: "ฉบับร่าง",
      closed: "ปิดแล้ว",
      singleDay: "กิจกรรมวันเดียว",
      multiDay: "กิจกรรมหลายวัน",
      noResults: "ไม่พบผลลัพธ์",
      showing: "แสดง",
      of: "จาก",
      pending: "รอตรวจสอบ",
      confirmDelete: "ยืนยันการลบ?",
      deleteEventText: "คุณต้องการลบกิจกรรมนี้หรือไม่?",
      deleted: "ลบสำเร็จ",
      eventDeletedSuccess: "ลบกิจกรรมเรียบร้อย",
      error: "เกิดข้อผิดพลาด",
      cannotDeleteEvent: "ไม่สามารถลบกิจกรรมได้",
      cancel: "ยกเลิก",
      back: "กลับ",
    ...
    en: {
      ...
      deleteEventText: "Do you want to delete this event?",
      deleted: "Deleted!",
      eventDeletedSuccess: "Event deleted successfully",
      error: "Error",
      cannotDeleteEvent: "Failed to delete event",
      cancel: "Cancel",
      back: "Back",
      ...
      finalizeRewards: "Finalize Rewards",
  ...
  function translateStatus(status: string): string {
    const map: any = { Active: lang.active, Closed: lang.closed, Draft: lang.draft };
  ...
  function getEventType(event: Event): 'single' | 'multi' {
    const start = new Date(event.startDate).toDateString();
    const end = new Date(event.endDate).toDateString();
    return start === end ? 'single' : 'multi';
  }
  
  function getEventTypeLabel(event: Event): string {
    return getEventType(event) === 'single' ? lang.singleDay : lang.multiDay;
  }
  
  function getParticipantPercentage(event: Event): number {
    if (!event.totalSlots) return 0;
    return Math.min((event.participants / event.totalSlots) * 100, 100);
  }
  
  function getPendingPercentage(event: Event): number {
    if (!event.pendingCount || !event.totalSlots) return 0;
    return Math.min((event.pendingCount / event.totalSlots) * 100, 100);
  }
  ...
  async function handleViewDetails(event: Event) {
    ...
    try {
      ...
      const rewardsResponse = await api.get(`/api/events/${event.id}/rewards`);
      if (selectedEvent) {
        selectedEvent.rewards = rewardsResponse.data.users || [];
      }
    } catch (error) {
    ...
    {:else}
          ...
          <div class="glass-card">
            <div class="card-img-wrapper">
              <img
                ...
                loading="lazy"
                on:error={(e) => {
                  e.currentTarget.src = 'https://placehold.co/400x200/1e293b/64748b?text=Image+Error';
                }}
              />
            ...
            <div class="card-body">
              ...
              <div class="card-content-area">
                ...
                <div class="event-simple-meta">
                  <div class="meta-row event-type">
                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                    </svg>
                    <span class="type-badge" class:single-day={getEventType(event) === 'single'} class:multi-day={getEventType(event) === 'multi'}>
                ...
                <div class="card-bottom-section">
                  <div class="participant-info">
                    <div class="participant-row">
                      ...
                      <span class="participant-label">{lang.participants}:</span>
                      <span class="participant-value">{event.participants || 0} / {event.totalSlots}</span>
                    </div>
                    <div class="progress-bar">
                      <div class="progress-fill" style="width: {getParticipantPercentage(event)}%;"></div>
                    </div>
                  </div>
                  ...
                  {#if event.pendingCount && event.pendingCount > 0}
                    <div class="participant-info pending">
                      <div class="participant-row">
                        ...
                        <span class="participant-label">{lang.pending}:</span>
                        <span class="participant-value">{event.pendingCount}</span>
                      </div>
                      <div class="progress-bar">
                        <div class="progress-fill pending" style="width: {getPendingPercentage(event)}%;"></div>
                      </div>
                    </div>
                  {/if}
                </div>
              ...
              <div class="card-actions-wrapper">
                <div class="action-buttons-full">
                  <button class="action-btn btn-view" on:click={() => handleViewDetails(event)}>
                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                  ...
                  <button class="action-btn btn-edit" on:click={() => handleEdit(event.id)}>
                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                  ...
                  <button class="action-btn btn-delete" on:click|stopPropagation={() => handleDelete(event.id)}>
                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
      ...
      {#if totalPages > 1}
        <div class="pagination-wrapper">
              ...
              {#if showPageDropdown}
                <div class="page-dropdown-list">
                  {#each Array(totalPages) as _, i}
                    <button class="page-option" class:active={currentPage === i + 1} on:click={() => jumpToPage(i + 1)}>
                      Page {i + 1}
                    </button>
                  {/each}
                </div>
          ...
          <div class="page-info">
            {lang.showing} {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, events.length)} {lang.of} {events.length}
          </div>
        </div>
      {/if}
    {/if}
  ...
  <div class="modal-overlay" on:click={closeDetailModal}>
    <div class="modal-container detail-modal" on:click|stopPropagation>
      <div class="modal-header">
        <h2>{lang.eventDetails}</h2>
        <button class="close-modal-btn" on:click={closeDetailModal}>
          ...
          {#if selectedEvent.image}
            <div class="detail-image">
              ...
              <div class="image-overlay">
                <span
                  class="status-badge"
                  class:status-active={selectedEvent.status === 'Active'}
                  class:status-closed={selectedEvent.status === 'Closed'}
                  class:status-draft={selectedEvent.status === 'Draft'}
                >
          ...
          <div class="info-section">
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                ...
                <p>{getEventTypeLabel(selectedEvent)}</p>
              </div>
              {#if selectedEvent.distanceKm}
                <div class="info-item">
                  <label>{lang.distance}</label>
                  <p>{selectedEvent.distanceKm} {lang.km}</p>
                </div>
              {/if}
            </div>
          </div>
          ...
          <div class="info-section">
            ...
            <div class="stats-cards">
              <div class="stat-card">
                ...
                <div class="stat-info">
                  <p class="stat-label">{lang.joined}</p>
                  <p class="stat-value">{selectedEvent.participants || 0}</p>
                </div>
              </div>
              ...
              <div class="stat-card">
                ...
                <div class="stat-info">
                  <p class="stat-label">{lang.totalSlots}</p>
                  <p class="stat-value">{selectedEvent.totalSlots}</p>
                </div>
              </div>

              <div class="stat-card">
                <div class="stat-icon" style="background: rgba(100, 116, 139, 0.1);">
                  <svg width="24" height="24" fill="none" stroke="#64748b" viewBox="0 0 24 24" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"></path>
                  </svg>
                </div>
                <div class="stat-info">
                  <p class="stat-label">{lang.available}</p>
                  <p class="stat-value">{selectedEvent.totalSlots - (selectedEvent.participants || 0)}</p>
                </div>
              </div>
            </div>
            ...
            <div class="progress-section">
              <div class="progress-info">
                <span>{lang.joined}: {selectedEvent.participants || 0} / {selectedEvent.totalSlots}</span>
                <span>{Math.round(getParticipantPercentage(selectedEvent))}%</span>
              </div>
              <div class="progress-bar large">
                <div class="progress-fill" style="width: {getParticipantPercentage(selectedEvent)}%;"></div>
              </div>
            </div>
          </div>
                ...
                {#each selectedEvent.rewards.slice(0, 10) as user}
                  <div class="reward-card">
                    ...
                    <div class="reward-info">
                      <p class="reward-name">{user.name}</p>
                      {#if user.tier}
                        <span class="reward-tier tier-{user.tier.toLowerCase()}">{user.tier}</span>
                      {/if}
                    </div>
                  </div>
                {/each}
      ...
      <div class="modal-footer">
        <button class="modal-btn btn-secondary" on:click={closeDetailModal}>
          {lang.close}
        </button>
        <button class="modal-btn btn-primary" on:click={() => { closeDetailModal(); handleEdit(selectedEvent.id); }}>
          <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
          </svg>
          {lang.edit}
...
<style>
  ...
  .participant-info.pending .participant-row svg {
    color: #f59e0b;
  }
